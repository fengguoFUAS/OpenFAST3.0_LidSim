...
!==================================================================================================================================
SUBROUTINE GetBladedLoggingChannels(u,p, xd, m, AddOuts,ErrStat, ErrMsg)
   ...
   CALL Fill_CONTROL_vars( 0.0_DbKi, u, p, LEN(ErrMsg), m%dll_data,AddOuts)
...
SUBROUTINE BladedInterface_CalcOutput(t, u, p, m, xd, AddOuts,ErrStat, ErrMsg)
   ...
   CALL Fill_CONTROL_vars( t, u, p,LEN(ErrMsg), m%dll_data ,AddOuts)
...
!! version 3.81.
SUBROUTINE Fill_avrSWAP( t, u, p,ErrMsgSz, dll_data ,AddOuts)
...
! Records L1 and onward are outputs [see Retrieve_avrSWAP()]
   ...
   DO I = 1,p%GatesPerBeam
       dll_data%avrSWAP( L + 2 + I ) = AddOuts%Vlos( I )
   ENDDO   
   
   !> Lidar roll, pitch and yaw tilt angular (rotational) displacement (deg) and velocities
   dll_data%avrSWAP(L + 2 + p%GatesPerBeam + 1) = AddOuts%LdrRoll
   dll_data%avrSWAP(L + 2 + p%GatesPerBeam + 2) = AddOuts%LdrPitch
...
SUBROUTINE Fill_CONTROL_vars( t, u, p,ErrMsgSz, dll_data,AddOuts )
...
!   REAL(SiKi),                     INTENT(INOUT)  :: avrSWAP(:)  ! the SWAP array for the Bladed DLL Interface
   ...
   call Fill_avrSWAP( t, u, p, ErrMsgSz, dll_data,AddOuts) ! we'll set the avrSWAP variable, for the legacy version of the DLL, too.
