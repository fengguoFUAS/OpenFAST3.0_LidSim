...
SUBROUTINE FAST_InitializeAll( t_initial, p_FAST, y_FAST, m_FAST, ED, BD, SrvD, AD14, AD, IfW, OpFM, SC_DX, HD, SD, ExtPtfm, &
   ...
   TYPE(IceDyn_Data),        INTENT(INOUT) :: IceD                !< All the IceDyn data used in time-
...
!   p_FAST%CheckHSSBrTrqC = .false.
   ...
   IF ( p_FAST%CompLidar == Module_LidSim ) THEN 
      IF ( p_FAST%CompInflow == Module_IfW ) THEN
         ...
         InitInData_LidSim%DT            =   p_FAST%DT
             ...
             IF ( p_FAST%CompAero == Module_AD14 ) THEN 
          		    LIDSIM%P%NBLADE	= NUMBL
                    LIDSIM%P%NELM	= AD14%P%ELEMENT%NELM  
                    LIDSIM%P%AeroynMode   = 14
              ELSEIF ( p_FAST%CompAero == Module_AD ) THEN
                    LIDSIM%P%NBLADE	= NUMBL
                    LIDSIM%P%NELM	= AD%P%ROTORS(1)%BEMT%NUMBLADENODES
                    LIDSIM%P%AeroynMode   = 15
              END IF
         
         CALL LidarSim_Init(InitInData_LidSim, LidSim%y, LidSim%p, IFW%p,InitOutData_LidSim, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2,ErrMsg2,ErrStat,ErrMsg,RoutineName)
   ...
   IF ( p_FAST%CompServo == Module_SrvD ) THEN
      ...
      CALL SrvD_Init( Init%InData_SrvD, SrvD%Input(1), SrvD%p, SrvD%x(STATE_CURR), SrvD%xd(STATE_CURR), SrvD%z(STATE_CURR), &
                      SrvD%OtherSt(STATE_CURR), SrvD%y, SrvD%m,SrvD%AddOuts,p_FAST%dt_module( MODULE_SrvD ), Init%OutData_SrvD, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2,ErrMsg2,ErrStat,ErrMsg,RoutineName)
...
!      Init%InData_FEAM%depth       =  Init%OutData_HD%WtrDpth    ! This need to be set according to the water depth in HydroDyn
   ...
   CALL FAST_InitOutput( p_FAST, y_FAST, Init, InitOutData_LidSim,ErrStat2, ErrMsg2 )
      CALL SetErrStat(ErrStat2,ErrMsg2,ErrStat,ErrMsg,RoutineName)
...
SUBROUTINE FAST_InitOutput( p_FAST, y_FAST, Init,InitOutData_LidSim, ErrStat, ErrMsg )
   ...
   TYPE(LidarSim_InitOutputType),  INTENT(IN)           :: InitOutData_LidSim                    !< Initialization output for LidSim  should be merged to Init later
...
SUBROUTINE FAST_ReadPrimaryFile( InputFile, p, m_FAST, OverrideAbortErrLev, ErrStat, ErrMsg )
   ...
   ! CompLidar - compute nacelle lidar measurement
...
SUBROUTINE ExitThisProgram_T( Turbine, ErrLevel_in, StopTheProgram, ErrLocMsg, SkipRunTimeMsg )
   ...
   IF (PRESENT(ErrLocMsg)) THEN
      ...
      CALL ExitThisProgram( Turbine%p_FAST, Turbine%y_FAST, Turbine%m_FAST, &
                     ...
                     Turbine%HD, Turbine%SD, Turbine%ExtPtfm, Turbine%MAP, Turbine%FEAM, Turbine%MD, Turbine%Orca, &
                     Turbine%IceF, Turbine%IceD, Turbine%MeshMapData, Turbine%LidSim,ErrLevel_in, StopTheProgram, ErrLocMsg, SkipRunTimes )
   ...
   ELSE
      ...
      CALL ExitThisProgram( Turbine%p_FAST, Turbine%y_FAST, Turbine%m_FAST, &
                     Turbine%ED, Turbine%BD, Turbine%SrvD, Turbine%AD14, Turbine%AD, Turbine%IfW, Turbine%OpFM, &
                     Turbine%HD, Turbine%SD, Turbine%ExtPtfm, Turbine%MAP, Turbine%FEAM, Turbine%MD, Turbine%Orca, &
                     Turbine%IceF, Turbine%IceD, Turbine%MeshMapData,Turbine%LidSim, ErrLevel_in, StopTheProgram, SkipRunTimeMsg=SkipRunTimes )
...
SUBROUTINE ExitThisProgram( p_FAST, y_FAST, m_FAST, ED, BD, SrvD, AD14, AD, IfW, OpFM, HD, SD, ExtPtfm, &
                            MAPp, FEAM, MD, Orca, IceF, IceD, MeshMapData,LidSim, ErrLevel_in, StopTheProgram, ErrLocMsg, SkipRunTimeMsg )
!...............................................................................................................................
